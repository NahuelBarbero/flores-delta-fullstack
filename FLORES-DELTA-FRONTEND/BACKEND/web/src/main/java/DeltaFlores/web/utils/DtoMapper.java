...
public final class DtoMapper {
    ...
    public static PlantEventDto plantEventToPlantEventDto(PlantEvent event) {
        ...
        } else if (event instanceof PhotoEvent) {
            return photoEventToPhotoEventDto((PhotoEvent) event);
        }
    ...
    public static PhotoEventDto photoEventToPhotoEventDto(PhotoEvent event) {
        PhotoEventDto dto = new PhotoEventDto();
        copyCommonEventPropertiesToDto(event, dto);
    ...
    private static void copyCommonEventPropertiesToDto(PlantEvent event, PlantEventDto dto) {
        ...
        if (event.getPlantas() != null) {
            dto.setPlantaIds(event.getPlantas().stream().map(Planta::getId).collect(Collectors.toList()));
        } else {
            dto.setPlantaIds(Collections.emptyList()); // Asegurar que no sea null
        }
    }
    ...
    public static PlantEvent plantEventDtoToPlantEvent(PlantEventDto eventDto, PlantEvent event) {
        ...
        } else if (eventDto instanceof PhotoEventDto) {
            return photoEventDtoToPhotoEvent((PhotoEventDto) eventDto,
                    (event instanceof PhotoEvent) ? (PhotoEvent) event : new PhotoEvent());
        }
    ...
    private static PhotoEvent photoEventDtoToPhotoEvent(PhotoEventDto dto, PhotoEvent event) {
        copyCommonEventPropertiesToEntity(dto, event);
        event.setDescription(dto.getDescription());
        if (dto.getMediaUrls() != null) {
            event.setMediaUrls(dto.getMediaUrls());
        }
        return event;
    }
